...
data = np.load('trajectories.npz')['trajectories'] # N*T
pairs = []
for n in range(data.shape[0]):
    for t in range(data.shape[1]-1):
        pairs.append([[data[n,t, :], data[n,t+1, :]]])
pairs = np.concatenate(pairs, axis=0)
print(pairs.shape) # 119*2*4
...
def get_reward(bs, actions, learnable_tree, tree_params, tree_optim):
    ...
    x = torch.from_numpy(pairs).float().cuda()
    # print(x.shape)
    ...
    for bs_idx in range(batch_size):
        ...
        for _ in range(20):
            loss = torch.nn.functional.mse_loss((x[:,1:2, whichDim]-x[:,0:1,whichDim])/dt, learnable_tree(x[:,0], bs_action))
            # print(((x[:,1:2, whichDim]-x[:,0:1,whichDim])/dt).shape, learnable_tree(x[:,0], bs_action).shape)
        ...
        # print('loss after, error: {}  '.format(regression_error.item()))
...
def best_error(best_action, learnable_tree):
    ...
    regression_error = torch.nn.functional.mse_loss((x[:,1:2, whichDim]-x[:,0:1,whichDim])/dt, learnable_tree(x[:,0], bs_action)) #func.LHS_pde(true(x), x, dim)#func.LHS_pde(learnable_tree(x, bs_action), x, dim)# func.LHS_pde(true(x), x, dim)#func.LHS_pde(learnable_tree(x, bs_action), x, dim)
...
def train_controller(Controller, Controller_optim, trainable_tree, tree_params, hyperparams):
    ...
    for step in range(hyperparams['controller_max_step']):
        ...
        # actions = [torch.tensor([0], dtype=torch.int32), torch.tensor([0], dtype=torch.int32), torch.tensor([2], dtype=torch.int32), torch.tensor([2], dtype=torch.int32)]
        # print(actions)
